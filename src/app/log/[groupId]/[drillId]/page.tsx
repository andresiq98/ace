...
function LogContent() {
    ...
    // Get duration from query params (passed from play page)
    ...
    const rivalPlayer = rivals.find(p => p.id === selectedRival);
    const canSave = winnerId && selectedScore && selectedRival;

    const iWon = winnerId === CURRENT_USER.id;
    const isMvp = mvpId === CURRENT_USER.id;
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar pb-6">
                ...
                <div className="flex gap-2.5 px-5 overflow-x-auto no-scrollbar mb-6 pb-1">
                    {rivals.map((p) => (
                        <button
                            key={p.id}
                            onClick={() => setSelectedRival(p.id)}
                            className="flex flex-col items-center gap-2 cursor-pointer shrink-0 transition-all duration-250"
                        >
                            <div
                                className={`w-14 h-14 rounded-full flex items-center justify-center font-display font-black text-base border-[2.5px] transition-all duration-250 ${selectedRival === p.id
                            ...
                            <span className={`text-[11px] font-semibold ${selectedRival === p.id ? "text-neon" : "text-ace-muted"
                                }`}>
                ...
                {selectedRival && (
                    <div className="flex items-center justify-center gap-4 px-5 pb-6 animate-slide-up">
                        <div className="text-center flex-1">
                            <div
                                className="w-[62px] h-[62px] rounded-full mx-auto mb-2.5 flex items-center justify-center font-display font-black text-xl border-[2.5px] border-neon"
                                style={{ background: CURRENT_USER.gradient, color: CURRENT_USER.textColor }}
                            >
                                {CURRENT_USER.initials}
                            </div>
                            <div className="font-bold text-sm">{CURRENT_USER.displayName.split(" ")[0]}</div>
                        ...
                        <div className="text-center flex-1">
                            <div
                                className="w-[62px] h-[62px] rounded-full mx-auto mb-2.5 flex items-center justify-center font-display font-black text-xl border-[2.5px] border-ace-border"
                                style={{ background: rivalPlayer?.gradient, color: rivalPlayer?.textColor }}
                            >
                                {rivalPlayer?.initials}
                            </div>
                            <div className="font-bold text-sm">{rivalPlayer?.displayName.split(" ")[0]}</div>
                        </div>
                    </div>
                )}
                ...
                {selectedRival && (
                    <>
                        ...
                        <div className="grid grid-cols-2 gap-2.5 px-5 mb-7">
                            <button
                                onClick={() => setWinnerId(CURRENT_USER.id)}
                                className={`bg-ace-surface border-2 rounded-2xl p-5 text-center cursor-pointer transition-all duration-250 active:scale-[0.96] ${winnerId === CURRENT_USER.id ? "border-neon bg-neon/[0.04]" : "border-ace-border"
                                    }`}
                            >
                                <div
                                    className="w-12 h-12 rounded-full mx-auto mb-2.5 flex items-center justify-center font-display font-black text-base"
                                    style={{ background: CURRENT_USER.gradient, color: CURRENT_USER.textColor }}
                                >
                                    {CURRENT_USER.initials}
                                </div>
                                <div className={`font-extrabold text-[15px] font-display uppercase tracking-[0.3px] ${winnerId === CURRENT_USER.id ? "text-neon" : ""}`}>
                            ...
                            >
                                ...
                                <div className={`font-extrabold text-[15px] font-display uppercase tracking-[0.3px] ${winnerId === selectedRival ? "text-neon" : ""}`}>
                                    {rivalPlayer?.displayName.split(" ")[0]}
                                </div>
                            </button>
                        </div>
                    </>
                ...
                {/* Step 3: Score — Dynamic by Duration */}
                {winnerId && (
                    <>
                ...
                {selectedScore && (
                    <>
                        ...
                        <div className="flex gap-2.5 px-5 overflow-x-auto no-scrollbar mb-6">
                            ...
                            <button
                                onClick={() => setMvpId(CURRENT_USER.id)}
                                className={`shrink-0 bg-ace-surface border-2 rounded-3xl px-4 py-2 flex items-center gap-2 font-display text-xs font-bold cursor-pointer transition-all duration-200 active:scale-95 ${mvpId === CURRENT_USER.id ? "border-ace-gold text-ace-gold bg-ace-gold/[0.06]" : "border-ace-border text-ace-muted"
                                    }`}
                            >
                                <div className="w-7 h-7 rounded-full flex items-center justify-center font-black text-[10px] shrink-0" style={{ background: CURRENT_USER.gradient, color: CURRENT_USER.textColor }}>
                                    {CURRENT_USER.initials}
                                </div>
                                {CURRENT_USER.displayName.split(" ")[0]}
                            </button>
                            {rivalPlayer && (
                                ...
                                >
                                    <div className="w-7 h-7 rounded-full flex items-center justify-center font-black text-[10px] shrink-0" style={{ background: rivalPlayer.gradient, color: rivalPlayer.textColor }}>
                                        {rivalPlayer.initials}
                                    </div>
                                    {rivalPlayer.displayName.split(" ")[0]}
                                </button>
                ...
                <div className="px-5">
                    <button
                        disabled={!canSave}
                        onClick={() => router.push(`/share/${groupId}/${drillId}?score=${selectedScore}&winner=${winnerId}&rival=${selectedRival}&duration=${duration}`)}
